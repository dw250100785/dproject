...
class dproject_task_type(osv.osv):
    _name = 'dproject.task.type'
    ...
    _columns = {
        ...
        #  多对多
        'project_ids': fields.many2many('dproject.project', 'dproject_task_type_rel', 'type_id', 'project_id', 'Projects'),
...
# ????  项目管理  _d
#column "use_tasks" of relation "dproject_project" does not exist

class dproject(osv.osv):
    # class 如何影响 _name
    #super(class_name,self)

    _name = "dproject.project"
    _description = "dProject"
    #继承自account.analytic.account，所以继承了 use_tasks  唉 伤不起啊
    #多重继承 什么意思啊
    ...
    def search(self, cr, user, args, offset=0, limit=None, order=None, context=None, count=False):
        if user == 1:
            return super(dproject, self).search(cr, user, args, offset=offset, limit=limit, order=order, context=context, count=count)
        if context and context.get('user_preference'):
                cr.execute("""SELECT project.id FROM dproject_project project
                           LEFT JOIN dproject_user_rel rel ON rel.project_id = project.id
                           WHERE rel.uid = %s """%(user,))
                return [(r[0]) for r in cr.fetchall()]
        return super(dproject, self).search(cr, user, args, offset=offset, limit=limit, order=order,
    ...
    def _get_projects_from_tasks(self, cr, uid, task_ids, context=None):
        tasks = self.pool.get('dproject.task').browse(cr, uid, task_ids, context=context)
        project_ids = [task.project_id.id for task in tasks if task.project_id]
        return self.pool.get('dproject.project')._get_project_and_parents(cr, uid, project_ids, context)
    ...
    def _get_project_and_parents(self, cr, uid, ids, context=None):
        ...
        while ids:
            cr.execute("""
                ...
                FROM dproject_project project, dproject_project parent
    ...
    def _get_project_and_children(self, cr, uid, ids, context=None):
        ...
        while ids:
            #  注意这种sql查询 SELECT project.id, parent.id
            cr.execute("""
    ...
    def _progress_rate(self, cr, uid, ids, names, arg, context=None):
        ...
        cr.execute("""
            ...
            FROM dproject_task WHERE project_id IN %s AND state <> 'cancelled'
            GROUP BY project_id
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        res =  super(dproject, self).unlink(cr, uid, ids, context=context)
    ...
    def _get_attached_docs(self, cr, uid, ids, field_name, arg, context):
        ...
        task = self.pool.get('dproject.task')
        for id in ids:
            #  这里注意  res_model
            project_attachments = attachment.search(cr, uid, [('res_model', '=', 'dproject.project'), ('res_id', '=', id)], context=context, count=True)
            task_ids = task.search(cr, uid, [('project_id', '=', id)], context=context)
            task_attachments = attachment.search(cr, uid, [('res_model', '=', 'dproject.task'), ('res_id', 'in', task_ids)], context=context, count=True)
    ...
    def _task_count(self, cr, uid, ids, field_name, arg, context=None):
        ...
        task_ids = self.pool.get('dproject.task').search(cr, uid, [('project_id', 'in', ids)], context=ctx)
        for task in self.pool.get('dproject.task').browse(cr, uid, task_ids, context):
            res[task.project_id.id] += 1
        return res

    def _get_alias_models(self, cr, uid, context=None):
        """Overriden in project_issue to offer more options"""
        return [('dproject.task', "Tasks")]
    ...
    def attachment_tree_view(self, cr, uid, ids, context):
        #  注意这里 res_model
        task_ids = self.pool.get('dproject.task').search(cr, uid, [('project_id', 'in', ids)])
        domain = [
             '|', 
             '&', ('res_model', '=', 'dproject.project'), ('res_id', 'in', ids),
             '&', ('res_model', '=', 'dproject.task'), ('res_id', 'in', task_ids)
		]
        ...
        return {
            ...
            'view_type': 'form',
            'limit': 80,
            'context': "{'default_res_model': '%s','default_res_id': %d}" % (self._name, res_id)#default_res_model   == self._name
        }
    # Lambda indirection method to avoid passing a copy of the overridable method when declaring the field
    _alias_models = lambda self, *args, **kwargs: self._get_alias_models(*args, **kwargs)
    _visibility_selection = lambda self, *args, **kwargs: self._get_visibility_selection(*args, **kwargs)

#use_tasks 一旦选中 就会出现  阶段选择
    _columns = {
        #'use_tasks': fields.boolean('use_tasks'),
        ...
        # financial management on projects. It enables you to connect projects with budgets, planning, cost and revenue analysis, timesheets on projects, etc
        #项目财务管理
        ...
        'tasks': fields.one2many('dproject.task', 'project_id', "Task Activities"),
        'planned_hours': fields.function(_progress_rate, multi="progress", string='Planned Time', help="Sum of planned hours of all tasks related to this project and its child projects.",
            store = {
                'dproject.project': (_get_project_and_parents, ['tasks', 'parent_id', 'child_ids'], 10),
                'dproject.task': (_get_projects_from_tasks, ['planned_hours', 'remaining_hours', 'work_ids', 'state'], 20),
            }),
        ...
        # 多对多
        'type_ids': fields.many2many('dproject.task.type', 'dproject_task_type_rel', 'project_id', 'type_id', 'Tasks Stages', states={'close':[('readonly',True)], 'cancelled':[('readonly',True)]}),
    ...
    def _get_type_common(self, cr, uid, context):
        ids = self.pool.get('dproject.task.type').search(cr, uid, [('case_default','=',1)], context=context)
        return ids
    ...
    _defaults = {
        ...
        'alias_model': 'dproject.task',#新建项目时用到
    ...
    def set_done(self, cr, uid, ids, context=None):
        task_obj = self.pool.get('dproject.task')
    ...
    def map_tasks(self, cr, uid, old_project_id, new_project_id, context=None):
        ...
        self.write(cr, uid, [new_project_id], {'tasks':[(6,0, map_task_id.values())]})#写入 多对多数据
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        res = super(dproject, self).copy(cr, uid, id, default, context)
    ...
    def setActive(self, cr, uid, ids, value=True, context=None):
        ...
        for proj in self.browse(cr, uid, ids, context=None):
            ...
            cr.execute('select id from dproject_task where project_id=%s', (proj.id,))
...
""" %   ( vacation, )
    ...
    def schedule_tasks(self, cr, uid, ids, context=None):
        ...
        for project in projects:
            ...
            # _generate_task  第三个参数是 对象
            result += self.pool.get('dproject.task')._generate_task(cr, uid, project.tasks, ident=4, context=context)
        ...
        for project in projects:
            ...
            for task in project.tasks:
                ...
                self.pool.get('dproject.task').write(cr, uid, [task.id], {
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if not vals.get('alias_id') and vals.get('name', False):
            ...
            alias_id = mail_alias.create_unique_alias(cr, uid,
                          ...
                          {'alias_name': "dproject+"+short_name(vals['name'])},
                          model_name=vals.get('alias_model', 'dproject.task'),  #这个是干嘛的 ？？？？
                          context=context)
            vals['alias_id'] = alias_id
        if vals.get('type', False) not in ('template','contract'):
            vals['type'] = 'contract'
        project_id = super(dproject, self).create(cr, uid, vals, context)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if vals.get('alias_model'):
            model_ids = self.pool.get('ir.model').search(cr, uid, [('model', '=', vals.get('alias_model', 'dproject.task'))])
            vals.update(alias_model_id=model_ids[0])
        return super(dproject, self).write(cr, uid, ids, vals, context=context)
...
class task(base_stage, osv.osv):
    ...
    #task 定义消息 并自动发送消息
    _track = {
        #  _track  来自 base_stage 对象
        'state': {
            'dproject.mt_dtask_new': lambda self, cr, uid, obj, ctx=None: obj['state'] in ['new', 'draft'],
            'dproject.mt_dtask_started': lambda self, cr, uid, obj, ctx=None: obj['state'] == 'open',
            'dproject.mt_dtask_closed': lambda self, cr, uid, obj, ctx=None: obj['state'] == 'done',
        },
        'stage_id': {
            'dproject.mt_dtask_stage': lambda self, cr, uid, obj, ctx=None: obj['state'] not in ['new', 'draft', 'done', 'open'],
        },
        'kanban_state': {  # kanban state: tracked, but only block subtype
            'dproject.mt_dtask_blocked': lambda self, cr, uid, obj, ctx=None: obj['kanban_state'] == 'blocked',
        },
    }
    ...
    def _get_default_partner(self, cr, uid, context=None):
        ...
        if project_id:
            project = self.pool.get('dproject.project').browse(cr, uid, project_id, context=context)
    ...
    def _resolve_project_id_from_context(self, cr, uid, context=None):
        ...
        if isinstance(context.get('default_project_id'), basestring):
            ...
            project_ids = self.pool.get('dproject.project').name_search(cr, uid, name=project_name, context=context)
            if len(project_ids) == 1:
                return project_ids[0][0]
        return None

    def _read_group_stage_ids(self, cr, uid, ids, domain, read_group_order=None, access_rights_uid=None, context=None):
        stage_obj = self.pool.get('dproject.task.type')
    ...
    def _read_group_user_id(self, cr, uid, ids, domain, read_group_order=None, access_rights_uid=None, context=None):
        ...
        access_rights_uid = access_rights_uid or uid
        if project_id:
            ids += self.pool.get('dproject.project').read(cr, access_rights_uid, project_id, ['members'], context=context)['members']
    ...
    def _hours_get(self, cr, uid, ids, field_names, args, context=None):
        res = {}
        cr.execute("SELECT task_id, COALESCE(SUM(hours),0) FROM dproject_task_work WHERE task_id IN %s GROUP BY task_id",(tuple(ids),))
    ...
    def _get_task(self, cr, uid, ids, context=None):
        result = {}
        for work in self.pool.get('dproject.task.work').browse(cr, uid, ids, context=context):
    ...
    _columns = {
        ...
        'stage_id': fields.many2one('dproject.task.type', 'Stage', track_visibility='onchange',
        ...
        'state': fields.related('stage_id', 'state', type="selection", store=True,
                ...
                help='The status is set to \'Draft\', when a case is created.\
                      ...
                      set to \'Pending\'.'),
        'categ_ids': fields.many2many('dproject.category', string='Tags'),
        ...
        'project_id': fields.many2one('dproject.project', 'Project', ondelete='set null', select="1", track_visibility='onchange'),
        #任务的嵌套
        'parent_ids': fields.many2many('dproject.task', 'dproject_task_parent_rel', 'task_id', 'parent_id', 'Parent Tasks'),
        'child_ids': fields.many2many('dproject.task', 'dproject_task_parent_rel', 'parent_id', 'task_id', 'Delegated Tasks'),
        'notes': fields.text('Notes'),
        'planned_hours': fields.float('限定时间', help='Estimated time to do the task, usually set by the project manager when the task is in draft state.'),
        # 好复杂的function
        # 了解 store功能
        #
        #
        'effective_hours': fields.function(_hours_get, string='Hours Spent', multi='hours', help="Computed using the sum of the task work done.",
            store = {
                'dproject.task': (lambda self, cr, uid, ids, c={}: ids, ['work_ids', 'remaining_hours', 'planned_hours'], 10),
                'dproject.task.work': (_get_task, ['hours'], 10),
            }),
        ...
        'progress': fields.function(_hours_get, string='Progress (%)', multi='hours', group_operator="avg", help="If the task has a progress of 99.99% you should close the task if it's finished or reevaluate the time",
            store = {
                'dproject.task': (lambda self, cr, uid, ids, c={}: ids, ['work_ids', 'remaining_hours', 'planned_hours','state'], 10),
                'dproject.task.work': (_get_task, ['hours'], 10),
            }),
        ...
        'user_id': fields.many2one('res.users', '分配给', track_visibility='onchange'),#修改发送通知
        'delegated_user_id': fields.related('child_ids', 'user_id', type='many2one', relation='res.users', string='Delegated To'),
        'partner_id': fields.many2one('res.partner', '客户'),
        'work_ids': fields.one2many('dproject.task.work', 'task_id', 'Work done'),
        'manager_id': fields.related('project_id', 'analytic_account_id', 'user_id', type='many2one', relation='res.users', string='项目经理'),
        'company_id': fields.many2one('res.company', '公司'),
        'id': fields.integer('ID', readonly=True),
        'color': fields.integer('Color Index'),
        'user_email': fields.related('user_id', 'email', type='char', string='电子邮件', readonly=True),
    }
    _defaults = {
        ...
        # 计算公司id
        'company_id': lambda self, cr, uid, ctx=None: self.pool.get('res.company')._company_default_get(cr, uid, 'dproject.task', context=ctx),
        'partner_id': lambda self, cr, uid, ctx=None: self._get_default_partner(cr, uid, context=ctx),
    }
    _order = "priority, sequence, date_start, name, id"#定义 多个order 主次
    ...
    def stage_find(self, cr, uid, cases, section_id, domain=[], order='sequence', context=None):
        ...
        stage_ids = self.pool.get('dproject.task.type').search(cr, uid, search_domain, order=order, context=context)
        if stage_ids:
            return stage_ids[0]
        return False
    ...
    def _store_history(self, cr, uid, ids, context=None):
        for task in self.browse(cr, uid, ids, context=context):
            self.pool.get('dproject.task.history').create(cr, uid, {
                'task_id': task.id,
...
''' % (ident, 'User_'+str(task.user_id.id))
    ...
    def project_task_reevaluate(self, cr, uid, ids, context=None):
        if self.pool.get('res.users').has_group(cr, uid, 'dproject.group_time_work_estimation_dtasks'):
            return {
                'view_type': 'form',
                "view_mode": 'form',
                'res_model': 'dproject.task.reevaluate',
                'type': 'ir.actions.act_window',
                'target': 'new',
            }
        return self.do_reopen(cr, uid, ids, context=context)

class dproject_work(osv.osv):
    _name = "dproject.task.work"
    _description = " Designer Project Task Work"
    _columns = {
        'name': fields.char('Work summary', size=128),
        'date': fields.datetime('Date', select="1"),
        'task_id': fields.many2one('dproject.task', 'Task', ondelete='cascade', required=True, select="1"),
    ...
    def create(self, cr, uid, vals, *args, **kwargs):
        if 'hours' in vals and (not vals['hours']):
            vals['hours'] = 0.00
        if 'task_id' in vals:
            cr.execute('update dproject_task set remaining_hours=remaining_hours - %s where id=%s', (vals.get('hours',0.0), vals['task_id']))
        return super(dproject_work,self).create(cr, uid, vals, *args, **kwargs)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if 'hours' in vals:
            for work in self.browse(cr, uid, ids, context=context):
                cr.execute('update dproject_task set remaining_hours=remaining_hours - %s + (%s) where id=%s', (vals.get('hours',0.0), work.hours, work.task_id.id))
        return super(dproject_work,self).write(cr, uid, ids, vals, context)

    def unlink(self, cr, uid, ids, *args, **kwargs):
        for work in self.browse(cr, uid, ids):
            cr.execute('update dproject_task set remaining_hours=remaining_hours + %s where id=%s', (work.hours, work.task_id.id))
        return super(dproject_work,self).unlink(cr, uid, ids,*args, **kwargs)

#这个必须在 dproject_project 之前  因为扩展了use_tasks   后又被继承了

class account_analytic_account(osv.osv):
    #"use_tasks" of relation "dproject_project" does not exist
    ...
    def project_create(self, cr, uid, analytic_account_id, vals, context=None):
        ...
        #是否要创建项目
        #必须满足  use_tasks  为真  并且 project_creation_in_progress为真
...
class dproject_project(osv.osv):
    _inherit = 'dproject.project'
    _defaults = {
    }

class dproject_task_history(osv.osv):
    """
    Tasks History, used for cumulative flow charts (Lean/Agile)
    """
    _name = 'dproject.task.history'
    ...
    _columns = {
        'task_id': fields.many2one('dproject.task', 'Task', ondelete='cascade', required=True, select=True),
        'type_id': fields.many2one('dproject.task.type', 'Stage'),
        ...
        'end_date': fields.function(_get_date, string='End Date', type="date", store={
            'dproject.task.history': (_get_related_date, None, 20)
        }),
...
class dproject_task_history_cumulative(osv.osv):
    _name = 'dproject.task.history.cumulative'
    _table = 'dproject_task_history_cumulative'
    _inherit = 'dproject.task.history'
    _auto = False

    _columns = {
        'end_date': fields.date('End Date'),
        'project_id': fields.many2one('dproject.project', 'Project'),
    }

    def init(self, cr):
        tools.drop_view_if_exists(cr, 'dproject_task_history_cumulative')

        cr.execute(""" CREATE VIEW dproject_task_history_cumulative AS (
            SELECT
            ...
            FROM (
                SELECT
                    ...
                    t.project_id
                FROM
                    dproject_task_history AS h
                    JOIN dproject_task AS t ON (h.task_id = t.id)

            ) AS history
        )
        """)

class dproject_category(osv.osv):
    """ Category of project's task (or issue) """
    _name = "dproject.category"
