...
class dproject_task_delegate(osv.osv_memory):
    _name = 'dproject.task.delegate'
    ...
    _columns = {
        ...
        'project_id': fields.many2one('dproject.project', 'Project', help="User you want to delegate this task to"),
    ...
    def onchange_project_id(self, cr, uid, ids, project_id=False, context=None):
        project_project = self.pool.get('dproject.project')
        if not project_id:
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        """
        res = super(dproject_task_delegate, self).default_get(cr, uid, fields, context=context)
        if context is None:
            context = {}
        record_id = context and context.get('active_id', False) or False
        if not record_id:
            return res
        task_pool = self.pool.get('dproject.task')
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        res = super(dproject_task_delegate, self).fields_view_get(cr, uid, view_id, view_type, context, toolbar, submenu=submenu)
    ...
    def delegate(self, cr, uid, ids, context=None):
        ...
        action_model, action_id = models_data.get_object_reference(cr, uid, 'dproject', 'action_view_dtask')
        view_model, task_view_form_id = models_data.get_object_reference(cr, uid, 'dproject', 'view_dtask_form2')
        view_model, task_view_tree_id = models_data.get_object_reference(cr, uid, 'dproject', 'view_dtask_tree2')
